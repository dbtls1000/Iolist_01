...
public class IolistService {
	...
	public void viewIolist() {
		...
		for(IolistVO vo : iolist) {
			...
			System.out.printf("%5d\t",vo.getIo_amt());
			int intTotal = vo.getIo_total();
			//숫자표현할때 3자리마다 컴마구분기호 넣기
...
//			
	...
	public boolean insertIO() {
		...
		IolistVO vo = new IolistVO(0,strDate,strPcode,strCcode,strInout,intAmt,intPrice,intTotal);
	...
	public boolean updateIO() {
		...
		String strDate = sd.format(date);
		System.out.println("==============================");
		System.out.println("거래정보수정");
		System.out.println("------------------------------");
		System.out.print("수정할 거래 >>");
		...
		/*
		 * 일단 거래처 코드를 보여주고
		 * 거래처 코드를 변경하려면 코드를 입력후 엔터
		 * 변경하지 않으려면 그냥 엔터
		 * 
		 * 코드를 입력하지 않고 엔터를 누르면
		 * strCcode에는 아무 값도 없다.
		 * 이 때 strCcode.isEmpty 는 true를 return한다
		 * 따라서 코드를 입력후 엔터를 누르면
		 * strCcode.isEmpty를 Skip하여 입력한 코드가 StrCcode에 담겨 있고
		 * 아무것도 입력하지 않고 엔터를 누르면
		 * strCcode.isEmpty가 true가 되어 vo에 담겨있는 io_ccode값을 변수에 대입하여
		 * 기존의 거래처코드를 유지한다.
		 */
		...
		System.out.printf("단가 (%s)>>",vo.getIo_price());
		String strPrice = scan.nextLine();
		int intPrice = 0;
		if(strPrice.isEmpty()) intPrice = vo.getIo_price();
		else intPrice = Integer.valueOf(strPrice);
		
		System.out.printf("수량 (%s)>>",vo.getIo_amt());
		String strAmt = scan.nextLine();
		int intAmt = 0;
		if(strAmt.isEmpty()) intAmt = vo.getIo_amt();
		else intAmt = Integer.valueOf(strAmt);
